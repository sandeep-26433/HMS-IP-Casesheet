...
class AppointmentBooking(models.Model):
    ...
    valid_upto_message = fields.Char(string="Validity Message", compute="_compute_valid_upto_message")
...
# Added functionality to search by phone number
   #  @api.onchange('phone')
   #  def _onchange_phone(self):
     #    """Auto-fill patient details based on phone number"""
      #   if self.phone:
            # Search for the patient by phone number
          #   patient = self.env['res.partner'].search([('phone', '=', self.phone)], limit=1)
          ...
          #   else:
                # If no matching patient found, clear the fields or leave them empty
            #     self.patient_id = False
              #   self.name = ""
              #   self.email = ""
                # Add a message to inform the user that no patient is linked with this phone number
              #   return {
                 #    'warning': {
                 #        'title': _('No Patient Found'),
                  #       'message': _('No patient is linked with this phone number.'),
                 #    }
               #  }
# Added functionality to search by phone number -END
    ...
    @api.depends('appointment_date', 'patient_id')
    def _compute_valid_upto(self):
        for record in self:
            if record.appointment_date and record.patient_id:
                # Find the most recent previous appointment for the same patient
                previous_appointment = self.search([
                    ('patient_id', '=', record.patient_id.id),
                ...
                if previous_appointment:
                    # Get its valid_upto
                    previous_valid_upto = previous_appointment.valid_upto

                    # Check if previous_valid_upto is a valid date (not None or False)
                    if previous_valid_upto:
                        # If the appointment date is after the previous valid_upto, recalculate valid_upto
                        if record.appointment_date > previous_valid_upto:
                            record.valid_upto = fields.Date.to_string(fields.Date.from_string(record.appointment_date) + timedelta(days=9))
                        else:
                            # If the appointment date is within the valid_upto period, keep the previous valid_upto
                            record.valid_upto = previous_appointment.valid_upto
                    else:
                        # If previous valid_upto is invalid, recalculate valid_upto based on the appointment date
                        record.valid_upto = fields.Date.to_string(fields.Date.from_string(record.appointment_date) + timedelta(days=9))
                else:
                    # If no previous appointment exists (first appointment), calculate valid_upto as 10 days from the appointment_date
                    record.valid_upto = fields.Date.to_string(fields.Date.from_string(record.appointment_date) + timedelta(days=9))
